      ...
      <div class="search-result-list">
        <a-table :columns="columns" :dataSource="dataSource" :loading="loading" :pagination="pagination" :scroll="{ x: 1500 }" bordered>
          ...
          <span slot="banner" slot-scope="bannerId">
            <img :src="'/api/download/infoBanner/' + bannerId" style="width: 50px; height: 50px" >
          </span>
...
<script>
import infoSearch from './Search.vue'
...
export default {
  ...
  methods: {
    ...
    edit (record) {
      if (record) {
        this.$router.push({ name: 'infoEdit', params: { id: parseInt(record.id) } })
      }
    },
    status (record, status) {
      if (record) {
        const _this = this
        confirm({
          title: '确认' + (status === 0 ? '禁用' : '启用') + '公告【' + record.title + '】？',
          content: '单击确认按钮执行此操作',
          onOk () {
            info.status(record.id, status).then(res => {
              if (res.data.state) {
                message.success('修改公告信息状态成功')
                _this.loader()
              } else {
    ...
    deleteData (record) {
      ...
      if (record) {
        confirm({
          ...
          onOk () {
            info.delete(record.id).then(res => {
              if (res.data.state) {
